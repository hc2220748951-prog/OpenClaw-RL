...
RUN_NAME="${RUN_NAME:-qwen3-0p6b-openclaw-rl-lora-safe}"
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/runs/${RUN_NAME}}"
mkdir -p "${RUN_ROOT}"

HF_CKPT="${HF_CKPT:-${REPO_ROOT}/models/Qwen3-0.6B}"
REF_LOAD="${REF_LOAD:-${HF_CKPT}}"
SAVE_CKPT="${SAVE_CKPT:-${REPO_ROOT}/ckpt/qwen3-0p6b-openclaw-rl-lora-safe}"
...
PORT="${PORT:-30010}"
RAY_PORT="${RAY_PORT:-16389}"
DASHBOARD_PORT="${DASHBOARD_PORT:-18275}"
...
export SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-qwen3-0.6b}"
export HOST="${HOST:-127.0.0.1}"
export OPENCLAW_RECORD_ENABLED="${OPENCLAW_RECORD_ENABLED:-1}"
export OPENCLAW_RECORD_FILE="${OPENCLAW_RECORD_FILE:-${RUN_ROOT}/qwen3_0p6b_lora_record.jsonl}"
export TP="${TP:-1}"
export CONTEXT_LENGTH="${CONTEXT_LENGTH:-16384}"
...
PRM_ARGS=(
  ...
  --prm-max-new-tokens "${PRM_MAX_NEW_TOKENS:-1024}"
)
...
if [ "${USE_WANDB}" = "1" ] && [ -n "${WANDB_KEY_VALUE}" ]; then
  WANDB_ARGS=(
    ...
    --wandb-group qwen3-0p6b-openclaw-rl-lora-safe
...
echo "Starting experimental 0.6B safe launcher with:"
